...
# Soft hang: gRPC/RNS handler не вернулся — процесс умирает, systemd Restart=always.
...
class DeviceControlBridge:
    ...
    def _run_handler_with_timeout(self, req: pb.CommandRequest) -> pb.CommandResponse:
        """Вызов backend с жёстким таймаутом — иначе soft hang держит клиента вечно."""
        box: dict = {}

        def run() -> None:
            try:
                box["resp"] = self._handler(req)
            except Exception as exc:  # noqa: BLE001 — отдаём клиенту, не роняем поток RNS
                box["err"] = exc
        ...
        if t.is_alive():
            ...
            # Немедленный выход: systemd Restart=always поднимет мост заново.
            os._exit(78)
        if "err" in box:
